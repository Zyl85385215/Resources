...
//#endif
			...
			case 11:
				...
				break;
			case 12:
				m_wLifeStarLv	= pRecv->dwVal;
				pMainMenu->m_pLifeStarSysMenu->SetStarLv(m_wLifeStarLv);
				break;
			case 13:
				m_dwExp += pRecv->dwVal*1000000000;
				break;
			}
	...
	case S_SEND_MAIL_INFO:
		{
			...
			if(pMainMenu->m_pMailMenu->m_pFormMail[MAILMENU_GETMAIL]->bOpened)
	...
	case S_SEND_HOME_MAXCTB:
		...
		return true;
	case S_FLAPPYBIRD_OPEN:
		{
			pMainMenu->m_pFlappyBirdMenu->StartGame();
		}
		break;
	case SC_CHECKTIME:
		{
			m_dwSCCheckTime	= xnGetTickCount();
		}
		return true;
	}
...
void UserData::UpdateSec()
{
	...
	if(m_dwSCCheckTime && xnGetTickCount() - m_dwSCCheckTime > 30000)
	{
		m_dwSCCheckTime	= 0;
		pChar->SendCmd(USER,SC_CHECKTIME);
	}
