...
void VipMenu::ResetPoint()
{
	...
	if(byPage > 0)
	{
		...
		for (int i = 0; i < pObj->pList->Count; i++)
		{
			...
			ItemExBase* pBase = ItemConfig::GetInstance().GetItemBase(nIDandNum/1000);
			if(pBase)
			{
				if (pBase->nStar == 3)
					m_pAwardIcon[i]->SetEffect(238,2);
				ShowItemInfoOnComp(m_pAwardIcon[i],pBase->nTypeID,nIDandNum%1000);
...
//	if(m_pForm && m_pForm->bOpened)
	{
		...
		if(pNext)
		{
			...
			m_pForm->GetComponent("jd")->SetProgress(100,100,"VIP等级已达上限");
		}
		ResetPoint();
	}
}
