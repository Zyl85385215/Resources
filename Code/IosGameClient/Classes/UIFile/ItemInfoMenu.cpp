...
// 		}
		...
		//m_pForm->GetComponent("shuxing3")->pSTextCenter->ClearSText();
		//m_pForm->GetComponent("shuxing4")->pSTextCenter->ClearSText();
		...
		if (m_pItemObj)
		{
			...
			for (int i=0;i<EQUIP_ATTR_MAX;i++)
			{
				if (nParambase[i])
				{
					...
					{
						sprintf(sztxt,"%s:%d",szAttribute[i],nParambase[i]);
						m_pForm->GetComponent("shuxing3")->pSTextCenter->AddSText(0,0,false,(char*)sztxt,255,255,255);
						if (m_pItemObj->GetStarLv())
						{
							sprintf(sztxt,"+%d",nParambase[i]*m_pItemObj->GetStarPer(m_pItemObj->GetStarLv())/10000);
							m_pForm->GetComponent("shuxing3")->pSTextCenter->AddSText(1,0,false,(char*)sztxt,nItemLimitColor[1][0],nItemLimitColor[1][1],nItemLimitColor[1][2]);
						}
						bFlagCnt++;
					}
					else if(bFlagCnt == 3)
					{
						sprintf(sztxt,"%s:%d",szAttribute[i],nParambase[i]);
						m_pForm->GetComponent("shuxing4")->pSTextCenter->AddSText(0,0,false,(char*)sztxt,255,255,255);
						if (m_pItemObj->GetStarLv())
						{
							sprintf(sztxt,"+%d",nParambase[i]*m_pItemObj->GetStarPer(m_pItemObj->GetStarLv())/10000);
							m_pForm->GetComponent("shuxing4")->pSTextCenter->AddSText(1,0,false,(char*)sztxt,nItemLimitColor[1][0],nItemLimitColor[1][1],nItemLimitColor[1][2]);
						}
						bFlagCnt++;
						break;
					}
			...
			if(m_pItemObj->pItemBase &&m_pItemObj->pItemBase->dwTimeLimit && m_nOpenStyle != FORM_MALL)
			{
				DWORD nTime = m_pItemObj->GetVal(ITEM_AD_LIMITTIME)- xnGetSecCount();
				m_pForm->GetComponent("pingfen2")->SetCaptionEx("剩余时间:%d天%d小时%d分",0,nTime/(24*3600),nTime%(24*3600)/3600,nTime%3600/60);
				m_pForm->GetComponent("pingfen2")->SetSprColor(255,0,0);
			}else
				m_pForm->GetComponent("pingfen2")->SetCaption("",0);
		}else{
				...
				if (pBase)
				{
					...
					for (int i=0;i<EQUIP_ATTR_MAX;i++){
						if (pBase->nBase[i]&&nFlagCnt == 0)
						{
							...
							nFlagCnt++;
						}else if (pBase->nBase[i]&&nFlagCnt == 1)
						{
							sprintf(sztxt,"%s:%d~%d",szAttribute[i],pBase->nBase[i],pBase->nBase[i]+pBase->nRandom[i]);
							m_pForm->GetComponent("shuxing2")->pSTextCenter->AddSText(0,0,true,(char*)sztxt,255,255,255)->nOffY -=3;
							nFlagCnt++;
						}
						else if (pBase->nBase[i]&&nFlagCnt == 2)
						{
							sprintf(sztxt,"%s:%d~%d",szAttribute[i],pBase->nBase[i],pBase->nBase[i]+pBase->nRandom[i]);
							m_pForm->GetComponent("shuxing3")->pSTextCenter->AddSText(0,0,true,(char*)sztxt,255,255,255)->nOffY -=3;
							nFlagCnt++;
						}
						else if (pBase->nBase[i]&&nFlagCnt == 3)
						{
							sprintf(sztxt,"%s:%d~%d",szAttribute[i],pBase->nBase[i],pBase->nBase[i]+pBase->nRandom[i]);
							m_pForm->GetComponent("shuxing4")->pSTextCenter->AddSText(0,0,true,(char*)sztxt,255,255,255)->nOffY -=3;
							break;
						}
					}
				}
			}
			if (m_pItemBase && m_pItemBase->bySlot == 9)
			{
				int nFlagCnt = 0;
				for (int i=0;i<EQUIP_ATTR_MAX;i++)
				{
					if (m_pItemBase->nParam[i]&&nFlagCnt == 0)
					{
						sprintf(sztxt,"%s:%d",szAttribute[i],m_pItemBase->nParam[i]);
						m_pForm->GetComponent("shuxing1")->pSTextCenter->AddSText(0,0,true,(char*)sztxt,255,255,255)->nOffY -=3;
						nFlagCnt++;
					}else if (m_pItemBase->nParam[i]&&nFlagCnt == 1)
					{
						sprintf(sztxt,"%s:%d",szAttribute[i],m_pItemBase->nParam[i]);
						m_pForm->GetComponent("shuxing2")->pSTextCenter->AddSText(0,0,true,(char*)sztxt,255,255,255)->nOffY -=3;
						nFlagCnt++;
					}
					else if (m_pItemBase->nParam[i]&&nFlagCnt == 2)
					{
						sprintf(sztxt,"%s:%d",szAttribute[i],m_pItemBase->nParam[i]);
						m_pForm->GetComponent("shuxing3")->pSTextCenter->AddSText(0,0,true,(char*)sztxt,255,255,255)->nOffY -=3;
						nFlagCnt++;
					}
					else if (m_pItemBase->nParam[i]&&nFlagCnt == 3)
					{
						sprintf(sztxt,"%s:%d",szAttribute[i],m_pItemBase->nParam[i]);
						m_pForm->GetComponent("shuxing4")->pSTextCenter->AddSText(0,0,true,(char*)sztxt,255,255,255)->nOffY -=3;
						break;
...
// 		strcat(sztxt2,"\r\n");
		...
		if (m_pItemObj && m_pItemObj->pItemBase)
		{
