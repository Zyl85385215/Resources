...
bool LifeStarSysMenu::CheckCanOpen()
{
	...
	{
		pMainMenu->PopTipMsg("点亮星辰达到90级开启日月系统");
		return false;
	}
	...
	if(bLv == pChar->m_wLifeStarLv+1 && bLv+90-1 <= pChar->m_byLvl)
	{
		...
		if(pStar)
		{
			...
			{				
				...
				{
					char szExp[64];
					sprintf(szExp, "需要消耗%u经验,可通过任务或者副本打怪获得。", pStar->nCostExp);
					pMainMenu->PopTipMsg(szExp);
					return;
				}
				if(pStar->nCostGold > pChar->m_dwMoney)
				{
					char szGold[64];
					sprintf(szGold, "需要消耗%u金币,可以点击日常参加活动快速获得金币。", pStar->nCostGold);
					pMainMenu->PopTipMsg(szGold);
					return;
				}
				char szTemp[128];
				sprintf(szTemp, "激活日月需要消耗%u金币和%u经验，是否激活。", pStar->nCostGold, pStar->nCostExp);
