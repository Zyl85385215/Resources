...
CCSprite* GFun_InitSpriteByJPG( char* szFile )
{
	...
	CCTexture2D*	pText2D	= CCTextureCache::sharedTextureCache()->addImage(szFile);
	if(pText2D)
	{
...
void Fun_SpriteSetSpr(char* szFile, void* pParam)
{
	...
	{
		CCTexture2D*	pText2D	= CCTextureCache::sharedTextureCache()->addImage(szTmp);
		if(pText2D)
...
void Fun_ChgSpriteByTag(char* szFile,int nPosX, int nPosY,int nTag, void* pParam, int nScale, int nRota,int zOrder)
{
	CObjSSprite*	pSpr	= (CObjSSprite*)pParam;
...
void MySetShader(SHADER_TYPE byType,void* pParam){
	...
	case SHADER_NULL:
		{
			pBtn->setShaderProgram(CCShaderCache::sharedShaderCache()->programForKey(kCCShader_PositionTextureColor));
			break;
		}
	case SHADER_GRAY:
		{
...
CObjSSprite::CObjSSprite( DComponent* pComp )
{
	...
	pComp->fnChgSprByTag  = Fun_ChgSpriteByTag;
...
// 			}
	...
	if(bNeedMove)
	{
		...
		//MovePosition(ptCur);
		CCAction * pCallback = CCCallFuncND::create(this, callfuncND_selector(FrameLayer::ActionCallBack),(void*)nStep); 
...
void FormLayer::_DCompCheck(CCNode* pFL)
{
	if(pFL->getChildren() && pFL->getChildren()->count() > 0)
	{
		...
		CCARRAY_FOREACH(pFL->getChildren(), child)
		{
			...
			{
				_DCompCheck(pNode);
			}else if (pNode->getTag () >= 8000 && pNode->getTag ()<10000)
			{
			...
			else if(pNode && pNode->getTag () != -1)
			{
				...
				pNode->setVisible (pComp->bVisable && pComp->m_bCocosVsb);
				if (pComp->bChgPos)
				{
					CCPoint	pt	= PtIF2Cocos (pComp->GetAlignmentX (),pComp->GetAlignmentY ());
...
bool FormLayer::Update()
{
	...
	//if(m_bBordeAdded)
	//{
	//	getChildByTag(9901)->setVisible(numberOfRunningActions()==0);
	//	//getChildByTag(9902)->setVisible(numberOfRunningActions()==0);
	//}
	_DCompCheck(this);
	if(m_bCloseAct)
		return true;
...
void FormLayer::_runCompAction( CCNode* pNode )
{
	...
	case 7:		//旋转
		{
			...
			CCAction * pCallback = CCCallFuncND::create(pNode, callfuncND_selector(FormLayer::ActionCallBack),(void*)7); 
	...
	case 8:		//逆时针旋转
		{
			CCActionInterval* pRota1	= CCRotateTo::create (1,-120);
			CCAction * pCallback = CCCallFuncND::create(pNode, callfuncND_selector(FormLayer::ActionCallBack),(void*)8); 
			CCFiniteTimeAction* pRun	= CCSequence::create (pRota1,pCallback,NULL);
			pNode->runAction( pRun);
		}
		break;
	case 11:		//选角色RPG
		{
			...
			CCAction * pCallback = CCCallFuncND::create(pNode, callfuncND_selector(FormLayer::ActionCallBack),(void*)11); 
	...
	case 15:			//计时用,2秒
		{
			...
			CCAction * pCallback = CCCallFuncND::create(pNode, callfuncND_selector(FormLayer::ActionCallBack),(void*)15); 
			CCAction * pCallback2 = CCCallFuncND::create(pNode, callfuncND_selector(FormLayer::ActionCallBack),(void*)16); 
...
//		}
		...
		pFrameLayer->m_bDisMoveTouch = m_pFData->pCFrame->bDisMoveTouch;
