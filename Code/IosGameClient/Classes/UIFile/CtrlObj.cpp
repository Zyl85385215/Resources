...
void CCtrlFPart::SetCocosVsb( bool bVsb )
{
	pCompBase->m_bCocosVsb	= bVsb;
	for (int i = 0; i < pListComps->Count; i++)
	{
		DComponent* pComp	= (DComponent*)pListComps->Items[i];
		pComp->m_bCocosVsb	= bVsb;
	}
}
...
void CCtrlFrame::CalcVisable()
{
	if(this == NULL)
		return;
	if(abs(nOffChgPos - nCheckChgPos) < 10)
		return;
	nCheckChgPos	= nOffChgPos;

	int nStartP		= nOffChgPos - 10;
	int nEndP		= nOffChgPos + nFrmMaxH + 10;
	if(bIsHoriz)
	{
		nStartP		= -nOffChgPos - 10;
		nEndP		= -nOffChgPos + nFrmMaxW + 10;
	}
	...
	for (int i = 0; i < pListFP->Count; i++)
	{
		...
		if(bIsHoriz)
		{
			if((pCopyFP->pCompBase->nX > nStartP && pCopyFP->pCompBase->nX < nEndP) ||
				pCopyFP->pCompBase->nX+pCopyFP->pCompBase->nW > nStartP && pCopyFP->pCompBase->nX+pCopyFP->pCompBase->nW < nEndP)
				pCopyFP->SetCocosVsb(true);
			else
				pCopyFP->SetCocosVsb(false);
		}
		else
		{
			if((pCopyFP->pCompBase->nY > nStartP && pCopyFP->pCompBase->nY < nEndP) ||
				pCopyFP->pCompBase->nY+pCopyFP->pCompBase->nH > nStartP && pCopyFP->pCompBase->nY+pCopyFP->pCompBase->nH < nEndP)
				pCopyFP->SetCocosVsb(true);
			else
				pCopyFP->SetCocosVsb(false);
		}
