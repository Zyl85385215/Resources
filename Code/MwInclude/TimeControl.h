...
/********************************************************************
	created:	2007/11/20
	created:	20:11:2007   10:27
	filename: 	E:\XXX\程序\测试\TimeControl\TimeControl.h
	file path:	E:\XXX\程序\测试\TimeControl
	file base:	TimeControl
	file ext:	h
...
public:
	...
	static tm		GetOffsetTime(int nOffsetVal);   //偏移值单位为：分
...
typedef  DWORD (*ONTIME)(CMTimeControl* pTime);
class CMTimeControl
{
public:
	CMTimeControl();
	...
	//-------------------------part of timer----------------
	void	SetEvent(ONTIME  pfunTime);
	int		StartTime(BYTE bType = timer_type_nul);
	void	StopTime();
	int		GetTimeCount();
private:
