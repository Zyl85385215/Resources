...
tm CMTime::GetOffsetTime(int nOffsetVal)
{
	time_t tmtTime = time(NULL) + nOffsetVal*60;
	return *(localtime(&tmtTime));	
}
...
// #endif
	...
	{
// #ifdef _DEBUG
// 		tm tm = pTime->m_tmStartTime.GetCurTime();
// 		printf(asctime(&tm));
// #endif	
		pTime->m_dwTimeCount ++;	
		if(pTime->m_pfunTime)
			pTime->m_pfunTime(pTime);
	}

}

int CMTimeControl::StartTime(BYTE bType)
{
	...
	case timer_type_hour:
		...
		else
		{
			...
			else
			{
				...
				m_dwTimerID		= timeSetEvent(3600000,1000,_TimeCallBack,(DWORD)this,TIME_PERIODIC);
			}
		}
		break;
	}
...
void CMTimeControl::SetEvent( ONTIME pfunTime )
{
	m_pfunTime = pfunTime;
}
