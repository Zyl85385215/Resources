...
void ChatMenu::Send()
{
	...
	if ((m_bChannel-1 == 0)&&(pChar->m_byLvl >= 20)&&(xnGetSecCount()-m_dwWCLmtTick>((60-((pChar->m_byLvl-20)/10)*10)>10?(60-((pChar->m_byLvl-20)/10)*10):10)))
	{
		m_dwWCLmtTick = xnGetSecCount();
	}else if ((m_bChannel-1 == 0)&&(pChar->m_byLvl >= 20)&&(xnGetSecCount()-m_dwWCLmtTick<((60-((pChar->m_byLvl-20)/10)*10)>10?(60-((pChar->m_byLvl-20)/10)*10):10)))
	{
		pMainMenu->PopTipMsg("%d秒后才可继续发言",((60-((pChar->m_byLvl-20)/10)*10)>10?(60-((pChar->m_byLvl-20)/10)*10):10)-(xnGetSecCount()-m_dwWCLmtTick));
		return;
	}else
		m_dwWCLmtTick = 0;
