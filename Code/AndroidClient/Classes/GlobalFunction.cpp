...
#ifdef USE_ANDROID
	char	szTmp[256]	= {0};
	sprintf(szTmp,"%s%s",CCFileUtils::sharedFileUtils()->getResourceDirectory(),szFile);
	if(access(szTmp,0) != -1)
		return true;
	return false;
#endif
#ifdef WIN32
	if(_access(CCFileUtils::sharedFileUtils()->fullPathFromRelativePath(szFile),0) != -1)
		return true;

	pMainMenu->PopTipMsg("缺少资源文件%s!!!",szFile);
	return false;
#endif
	if(access(CCFileUtils::sharedFileUtils()->fullPathFromRelativePath(szFile),0) == 0)
		return true;
	return false;
}
